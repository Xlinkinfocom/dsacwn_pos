    ...
    <div id="receipt-data">
        ...
        <table>
            <tbody>
                ...
                @endphp
                <tr><td colspan="2">{{$product_name}}<br>{{$product_sale_data->qty}} x {{number_format((float)(($product_sale_data->total - $product_sale_data->tax) / $product_sale_data->qty), 2, '.', '')}}</td>
                    <td style="text-align:right;vertical-align:bottom">{{number_format((float)(($product_sale_data->total - $product_sale_data->tax) / $product_sale_data->qty), 2, '.', '')}}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->total_price - $lims_sale_data->total_tax, 2, '.', '')}}</th>
                </tr>
                @if($lims_sale_data->total_tax)
                <tr>
                    <th colspan="2">{{trans('file.Order Tax')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->total_tax, 2, '.', '')}}</th>
                </tr>
                @endif
                <tr>
        ...
        <table>
            <tbody>
                <tr style="background-color:#ddd;">
                    <td style="padding: 5px;width:30%">{{trans('file.Paid By')}}: {{$lims_sale_data->paying_method}}</td>
                    <td style="padding: 5px;width:40%">{{trans('file.Amount')}}: {{number_format((float)$lims_sale_data->grand_total, 2, '.', '')}}</td>
                    <td style="padding: 5px;width:30%">{{trans('file.Change')}}: {{number_format((float)$lims_sale_data->quick_change, 2, '.', '')}}</td>
                </tr>
