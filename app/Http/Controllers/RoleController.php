...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('quick-sales-index')){
            $permission = Permission::firstOrCreate(['name' => 'quick-sales-index']);
            if(!$role->hasPermissionTo('quick-sales-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('quick-sales-index');

        if($request->has('quick-sales-add')){
            $permission = Permission::firstOrCreate(['name' => 'quick-sales-add']);
            if(!$role->hasPermissionTo('quick-sales-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('quick-sales-add');

        if($request->has('quick-sales-delete')){
            $permission = Permission::firstOrCreate(['name' => 'quick-sales-delete']);
            if(!$role->hasPermissionTo('quick-sales-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('quick-sales-delete');
