...
class SaleController extends Controller
{
    ...
    // quick sale view satrt

    public function view_quick_sale()
    {      
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('sales-index')) {
            $permissions = Role::findByName($role->name)->permissions;
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own'){
                $lims_sale_all = Quick_Sale::orderBy('id', 'desc')->where('user_id', Auth::id())->get();
            }
            else{
                $lims_sale_all = Quick_Sale::orderBy('id', 'desc')->get();
            }
            ...
            // $biller = Biller::where("id", '1')->first();
            // // echo $biller['name'];
            // echo $biller->name;
            // echo "<pre>";
            // print_r($lims_sale_all);
            // die();

            return view('sale.view_quick_sale',compact('lims_sale_all', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'all_permission'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }

    public function quickSaleData(Request $request)
    {
        $columns = array( 
            1 => 'created_at', 
            2 => 'reference_no',
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
            $totalData = Quick_Sale::where('user_id', Auth::id())->count();
        else
            $totalData = Quick_Sale::count();
        ...
        //     if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
        //         $sales = Quick_Sale::with('user_id', 'customer_name')->offset($start)
        ...
        //     if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
        //         $sales =  Quick_Sale::select('sales.*')
        //                     ->with('user_id', 'customer_name')
        ...
        //         $totalFiltered = Quick_Sale::whereDate('created_at', '=' , date('Y-m-d', strtotime(str_replace('/', '-', $search))))
        //                     ->where('user_id', Auth::id())
        //                     ->orwhere([
        //                         ['reference_no', 'LIKE', "%{$search}%"],
        //                         ['user_id', Auth::id()]
        ...
        //     }
        //     else {
        //         $sales =  Quick_Sale::select('*')
        //                     ->with('user_id', 'customer_name')->whereDate('created_at', '=' , date('Y-m-d', strtotime(str_replace('/', '-', $search))))
        //                     ->where('user_id', Auth::id())
        ...
        // if(empty($request->input('search.value'))){
        //     if(Auth::user()->role_id > 2 && config('staff_access') == 'own'){
        //         $sales = Quick_Sale::with('biller', 'warehouse', 'user')->offset($start)
        //                     ->where('user_id', Auth::id())
        //                     ->limit($limit)
        //                     ->orderBy($order, $dir)
        //                     ->get();
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own'){
            $sales = Quick_Sale::orderBy('id', 'desc')->where('user_id', Auth::id())->get();
        }
        else{
            $sales = Quick_Sale::orderBy('id', 'desc')->get();
        }
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                // $biller = Biller::where("id", $sale->user_id)->first();
                $biller = Biller::where("id", $sale->biller_id)->first();
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="'.route('sale.quick_invoice', ['id' => $sale->id]).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Invoice').'</a></li>
                                <li>
                ...
                $warehouse_name = '';
                $coupon_discount = '';
                $payment_method = $sale->paying_method;
                $payment_note = $sale->payment_note;
                $by_cash = $sale->by_cash;
                $by_card = $sale->by_card;
                $change = $sale->quick_change;

                $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' "'.$biller->name.'"', ' "'.$biller->company_name.'"', ' "'.$biller->email.'"', ' "'.$biller->phone_number.'"', ' "'.$biller->address.'"', ' "'.$biller->city.'"', ' "'.$sale->customer_name.'"', ' "'.$customer_phone_number.'"', ' "'.$customer_address.'"', ' "'.$customer_city.'"', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$order_tax.'"', ' "'.$order_tax_rate.'"', ' "'.$order_discount.'"', ' "'.$shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.$sale_note.'"', ' "'.$staff_note.'"', ' "'.$biller->name.'"', ' "'.$biller->email.'"', ' "'.$warehouse_name.'"', ' "'.$coupon_code.'"', ' "'.$coupon_discount.'"','"'.$payment_method.'"','"'.$payment_note.'"','"'.$by_cash.'"','"'.$by_card.'"','"'.$change.'"]'
                );
    ...
    public function quickProductSaleData($id)
    {
        $lims_product_sale_data = Quick_Product_Sale::where('sale_id', $id)->get();
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
           
            $discount = '0.00';

            $product_sale[0][$key] = $product_sale_data->product_name;
            $product_sale[1][$key] = $product_sale_data->qty;
            $product_sale[2][$key] = $product_sale_data->sale_unit;
            $product_sale[3][$key] = $product_sale_data->tax;
            $product_sale[4][$key] = $product_sale_data->tax_rate;
            $product_sale[5][$key] = $discount;
            $product_sale[6][$key] = $product_sale_data->total;
        }
        return $product_sale;
    }

    public function quickDeleteBySelection(Request $request)
    {
        $sale_id = $request['saleIdArray'];
        foreach ($sale_id as $id) {
            $lims_sale_data = Quick_Sale::find($id);
            $lims_product_sale_data = Quick_Product_Sale::where('sale_id', $id)->get();        
    ...
    function action(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            if($total_row > 0)
            {   
                $output .='<ul style="padding: 5px;background-color: #7c5cc4;color:white;border-radius: 5px;width: 400px;list-style-type:none;position: absolute;max-height: 100px;overflow-y: scroll;z-index: 9999;" id="srch_ul">';
                foreach($data as $row)
                {
                    $output .='<li style="padding: 8px 5px;border-bottom: 1px solid wheat;font-size: initial;" id="'.$n.'">'.$row->name.'</li>';
                }
                $output .='</ul>';
            }
    ...
    public function store(Request $request)
    {
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            $lims_payment_data->by_cash = $data['by_cash'];
            $lims_payment_data->by_card = $data['by_card'];
    ...
    public function quick_store(Request $request)
    {
        ...
        $data['reference_no'] = 'qckpos-' . date("Ymd") . '-'. date("his");

        $count = 0;
        $qty_count = 0;
        $total_tax = 0;
        foreach ($data['pro_name'] as $i => $pname) {
            $rate = $data['pro_tax'][$i];
            $pro_subtotal = $data['pro_subtotal'][$i];
            // $lims_tax_data = Tax::where('id', $tid)->first();
            // $rate = $lims_tax_data['rate'];
            $total_tax += round((($pro_subtotal*$rate)/(100+$rate)), 2);
            $qty_count += $data['pro_qty'][$i];
            $count++;
        }

        $paying_method = '';

        if($data['quick_paid_by_id_select'] == 1){
            $paying_method = 'Cash';
        }elseif ($data['quick_paid_by_id_select'] == 3){
            $paying_method = 'Credit Card';
        }elseif ($data['quick_paid_by_id_select'] == 7){
            $paying_method = 'Mix Payment';
        }

        $quick_change = round(($data['quick_paying_amount']-$data['pro_grand_total']), 2);
        
        $quick_data = array(
            'reference_no' => $data['reference_no'],
            'user_id' => $data['user_id'],
            'warehouse_id' => $data['warehouse_id_hidden'],
            'biller_id' => $data['biller_id_hidden'],
            'customer_name' => 'Walk-In-Customer',
            'item' => $count,
            'total_qty' => $qty_count,
            'total_tax' => $total_tax,
            'total_price' => $data['pro_grand_total'],
            'grand_total' => $data['pro_grand_total'],
            'sale_status' => '1',
            'payment_status' => '4',
            'paid_amount' => $data['pro_grand_total'],
            'by_cash' => $data['quick_by_cash'],
            'by_card' => $data['quick_by_card'],
            'quick_change' => $quick_change,
            'sale_note' => $data['quick_sale_note'],
            'staff_note' => $data['quick_staff_note'],
            'payment_note' => $data['quick_payment_note'],
            'paying_method' => $paying_method,
        );

        $lims_sale_data = Quick_Sale::create($quick_data);
        
        $sale_id = $lims_sale_data['id'];
        
        foreach ($data['pro_name'] as $i => $pname) {
            
            $pro_name = $data['pro_name'][$i];
            $pro_count = $data['pro_qty'][$i];
            $pro_unit = $data['pro_unit'][$i];
            $pro_price = $data['pro_price'][$i];
            $tax_rate = $data['pro_tax'][$i];
            $pro_subtotal = $data['pro_subtotal'][$i];
            // $lims_tax_data = Tax::where('id', $tid)->first();
            ...
            $tax = round((($pro_subtotal*$rate)/(100+$rate)), 2);
            
            $quick_pro_data = array(
                'sale_id' => $sale_id,
                'product_name' => $pro_name,
                'qty' => $pro_count,
                'sale_unit' => $pro_unit,
                'net_unit_price' => $pro_price,
                'tax_rate' => $tax_rate,
                'tax' => $tax,
                'total' => $pro_subtotal,
            );
            Quick_Product_Sale::create($quick_pro_data);
        ...
        if($lims_sale_data->sale_status == '1')
            return redirect('sales/gen_quick_invoice/'.$lims_sale_data->id)->with('message', $message);
        elseif($data['pos'])
            return redirect('pos')->with('message', $message);
        else
            return redirect('sales')->with('message', $message);
    ...
    public function genQuickInvoice($id)
    {
        $lims_sale_data = Quick_Sale::find($id);
        $lims_product_sale_data = Quick_Product_Sale::where('sale_id', $id)->get();
        $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        return view('sale.quick_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'numberInWords'));
    }
    ...
    public function genDemoInvoice()
    {
        // $lims_sale_data = Sale::find($id);
        // $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        // $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        // return view('sale.demo_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));

        return view('sale.demo_invoice');
    }
